...
constructor(ISuperFluid host, IConstantFlowAgreementV1 cfa, IInstantDistributionAgreementV1 ida, ISuperToken acceptedToken) public ERC721 ("emaNaFTe", "emNFT") {
    ...
    function bid(uint256 _bidRate) public payable returns (uint256, uint256, address) {
        Auction storage _auction = tokenIdToAuction[totalAuctions];
       
        require((now < _auction.endTime), "this auction is already over");
        require(_bidRate > _auction.highBid, "you must bid more than the current high bid");
        require(_acceptedToken.balance(msg.sender) >= (_bidRate * winLength), "your token balance isn't enough to win the auction" );
        ...
        _host.callAgreement(_cfa.address, _cfa.contract.methods.createFlow
           (_acceptedToken.address, address(this), _bidRate, "0x").encodeABI(), { from: msg.sender })
        ...
        _auction.highBid = _bidAmt;
        _auction.bids.push(_bidAmt);
    ...
    function claimNFT() public payable returns (uint) {
        ...
        _host.callAgreement(_cfa.address, _cfa.contract.methods.deleteFlow
             (_acceptedToken.address, msg.sender, address(this), _auction.highBid, "0x").encodeABI(), { from: msg.sender })
